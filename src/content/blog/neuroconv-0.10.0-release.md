...
- Images are chunked and compressed by default now, both the ones `ImageInterface` writes and the ophys summary images, where they had been written raw, so a file full of them is a fraction of the size it used to be. They also carry the dtype of the source, `uint8` or `uint16` as the case may be, rather than `float64`.
