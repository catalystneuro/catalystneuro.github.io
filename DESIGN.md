...
Funder logos on the homepage are shown as single-colour navy marks generated by
`scripts/mono-logos.mjs`, each with its own display height so that square and wide marks
carry similar visual weight. Add a new funder by adding it to that script and to
`PARTNERS` in `site-content.ts`, rather than by greyscaling the original in CSS.
...
- **Select chevron:** every `select` drops the platform arrow for a 14px hairline chevron in
  Soft Ink, set globally in `global.css`. Page styles must set `background-color`, not the
  `background` shorthand, or the chevron disappears.
